...
def step2():
    ...
    for index, row in df.iterrows():
        ...
        if tdf.empty:
            ...
            # if(tdf.count())
            # print(tdf['actual_vehicle_passenger_car_revenue_miles'].to_string(index=False))
            # newVal = int(tdf['actual_vehicle_passenger_car_revenue_miles'].to_string(index=False).replace(',', ''))+1000
            # print(newVal)
