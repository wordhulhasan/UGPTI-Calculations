...
def step9():
    df = pd.read_csv('Output/step8_opex_by_vrh.csv', dtype='str')
    df_2 = pd.read_csv('Output/step8_opex_by_vrh.csv', dtype='str')
    df = clean_names(df)
    # df = df.head(5)
    ...
    for index, row in df.iterrows():
        ...
        uptByVrmTotalCalculation(df, df_2, index)
    df.to_csv('Output/step9_trips_by_vrm.csv')


def uptByVrmTotalCalculation(df, df_2, index):
    ...
    if valVrmTotal == 0:
        df.loc[index, 'upt_by_vrm_total'] = 0
    else:
        df.loc[index, 'upt_by_vrm_total'] = "{:.2f}".format(valUptTotal / valVrmTotal)
