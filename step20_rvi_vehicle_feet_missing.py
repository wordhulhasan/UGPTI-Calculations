...
def step20():
    ...
    totalCalculation(df)
    df = clean_names(df)
    df.to_csv('Output/step20_rvi_vehicle_feet_missing.csv')
...
def totalCalculation(df):
    for index2, row2, in df.iterrows():
        ...
        total = float(str(row2['number_of_Bus_missing_feet'])) \
                + float(str(row2['number_of_Cutaway_missing_feet'])) \
                + float(str(row2['number_of_Van_missing_feet'])) \
                + float(str(row2['number_of_Minivan_missing_feet'])) \
                + float(str(row2['number_of_Automobile_missing_feet'])) \
                + float(str(row2['number_of_School Bus_missing_feet'])) \
                + float(str(row2['number_of_Over-the-road Bus_missing_feet'])) \
                + float(str(row2['number_of_Sports Utility Vehicle_missing_feet'])) \
                + float(str(row2['number_of_Aerial Tramway_missing_feet'])) \
                + float(str(row2['number_of_Articulated Bus_missing_feet'])) \
                + float(str(row2['number_of_Ferryboat_missing_feet'])) \
                + float(str(row2['number_of_Streetcar Rail_missing_feet'])) \
                + float(str(row2['number_of_Trolleybus_missing_feet'])) \
                + float(str(row2['number_of_Light Rail Vehicle_missing_feet'])) \
                + float(str(row2['number_of_Commuter Rail Self-Propelled Passenger Car_missing_feet'])) \
                + float(str(row2['number_of_Commuter Rail Passenger Coach_missing_feet'])) \
                + float(str(row2['number_of_Monorail Vehicle_missing_feet'])) \
                + float(str(row2['number_of_Double Decker Bus_missing_feet'])) \
                + float(str(row2['number_of_Commuter Rail Locomotive_missing_feet'])) \
                + float(str(row2['number_of_Heavy Rail Passenger Car_missing_feet'])) \
                + float(str(row2['number_of_Vintage Trolley_missing_feet'])) \
                + float(str(row2['number_of_Inclined Plain Vehicle_missing_feet'])) \
                + float(str(row2['number_of_Automated Guideway Vehicle_missing_feet'])) \
                + float(str(row2['number_of_Cable Car_missing_feet'])) \
                + float(str(row2['number_of_Other_missing_feet']))
        df.loc[index2, 'number_vehicle_missing_feet_total'] = total
...
def vehicle_calculation(df, df_rev_inventory, vehicletype):
    for index, row in df.iterrows():
        ...
        tdf = tdf[tdf['vehicle_length'].isna()]
        ...
        df.loc[index, 'number_of_' + vehicletype+"_missing_feet"] = sum
