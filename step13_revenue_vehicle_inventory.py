...
def step13():
    ...
    df.to_csv('Output/step13_revenue_vehicle_inventory.csv')


def vehicle_calculation(df, df_rev_inventory,mode):
    for index, row in df.iterrows():
        sum = 0
        tdf = df_rev_inventory.query("ntd_id== @index and vehicle_type == @mode")
        for openIndex, row in tdf.iterrows():
            sum = sum + int(row['active_fleet_vehicles'].replace(',', '').replace('$', ''))
        df.loc[index, 'number_of_'+mode] = sum
