...
def step10():
    df = pd.read_csv('../UGPTI-Calculations/Output/step9_trips_by_vrm.csv', dtype='str')
    df_2 = pd.read_csv('../UGPTI-Calculations/Output/step9_trips_by_vrm.csv', dtype='str')
    df = clean_names(df)
    df = df.head(5)
    ...
    for index, row in df.iterrows():
        df = totalTripsCalculationByVRH(df, df_2, index, ar, 0)
    ...
    df.to_csv('../UGPTI-Calculations/Output/step10_trips_by_vrh.csv')
...
def totalTripsCalculationByVRH(df, df_2, index, mode, flag):
    ...
    if valVrh == 0:
        ...
        df.loc[index, 'upt_' + mode.lower()+'_per_vrh'] = "{:.2f}".format(valTrips / valVrh)
    return df
